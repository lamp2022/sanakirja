...
Build en_it.json from EN keys + kaikki German + GT EN→IT.
...
- Extras merge from en_it_extras.json at end

Run: python3 41_build_en_it.py
"""
...
KAIKKI_FILE = "../sanakirja/kaikki_it_en_it.json"
GT_CHECKPOINT = "../sanakirja/gt_en_it_checkpoint.jsonl"
CLAUDE_FILE = "claude_en_it.jsonl"
FREQ_FILE = "../sanakirja/it_freq_50k.txt"
EXTRAS_FILE = "en_it_extras.json"
OUTPUT_FILE = "en_it.json"
REVIEW_FILE = "EN_IT_REVIEW_SAMPLE.md"
...
def load_it_freq(path: str) -> dict[str, int]:
    freq: dict[str, int] = {}
...
def strip_it_purpose(s: str) -> str:
    """Strip 'per ' purpose-clause prefix from GT output (Italian)."""
    s = s.strip()
    if s.lower().startswith("per "):
        return s[4:]
    return s


def looks_like_it_verb(s: str) -> bool:
    """Italian verb infinitives end in -are/-ere/-ire (and reflexive -arsi/-ersi/-irsi)."""
    s = strip_it_purpose(s).lower().strip()
    if not s or " " in s:
        return False
...
def _norm(s: str) -> str:
    """Normalize IT for comparison: lowercase, strip trailing reflexive 'si'."""
    s = s.lower().strip()
    # Strip leading articles common in GT output
...
def consensus_override(kaikki_primary: str, it_gt: str, claude_it: str) -> str | None:
    ...
    raw_candidates = [s for s in [it_gt, claude_it] if s]
...
def it_matches(candidate: str, gt_de: str) -> bool:
...
def select_kaikki_by_pos(entry: dict, en_word: str, it_freq: dict) -> list[str]:
    ...
    def sort_key(w):
        ...
        rank = it_freq.get(w.lower(), 99999)
    ...
    with open(path) as f:
        for line in f:
            ...
            try:
                ...
                if k:
                    cache[k] = rec.get("it_gt") or rec.get("it") or ""
...
def main():
    ...
    it_freq = load_it_freq(FREQ_FILE)
    print(f"DE freq list: {len(it_freq):,} words")
    ...
    print(f"kaikki EN→IT: {len(kaikki):,} EN entries")
    ...
    if todo_gt:
        print(f"\nRunning GT EN→IT for {len(todo_gt):,} new keys (≈{len(todo_gt)*GT_DELAY/60:.1f} min)…")
        from utils import gt
        with open(GT_CHECKPOINT, "a") as ckpt:
            for i, en in enumerate(todo_gt):
                if i % 200 == 0 and i > 0:
                    print(f"  {i:,}/{len(todo_gt):,} done")
                tr = gt(en, sl="en", tl="it")
                rec = {"en": en, "it_gt": tr or ""}
    ...
    for en in en_keys:
        ...
        raw_kaikki = select_kaikki_by_pos(kaikki_entry, en_norm, it_freq)
        kaikki_its = [d for d in raw_kaikki if is_lemma(d)][:3]
        has_kaikki = bool(kaikki_its)
        if has_kaikki:
            stats["has_kaikki"] += 1

        it_gt = gt_cache.get(en, "") or ""
        claude_it = claude_cache.get(en_norm, "")

        if has_kaikki:
            primary_it = kaikki_its[0]
            it_all = kaikki_its
            # Level 1: consensus override
            override = consensus_override(primary_it, it_gt, claude_it)
            if override:
                primary_it = override
                it_all = [override]
                stats["consensus_overridden"] = stats.get("consensus_overridden", 0) + 1
            # Level 3: GT-frequency override
            elif it_gt and it_freq:
                prim_n = _norm(primary_it).rstrip("!-").strip()
                gt_n = _norm(it_gt).rstrip("!-").strip()
                gt_root = gt_n.split()[0] if gt_n else ""
                if gt_root and " " not in prim_n:
                    prim_rank = it_freq.get(prim_n, 99999)
                    gt_rank = it_freq.get(gt_root, 99999)
                    if prim_rank > GT_FREQ_THRESHOLD and gt_rank * GT_FREQ_RATIO < prim_rank:
                        primary_it = gt_root if " " not in it_gt else it_gt
                        it_all = [primary_it]
                        stats["gt_freq_overridden"] = stats.get("gt_freq_overridden", 0) + 1
            # Level 4: verb-noun mismatch (EN 'to X', primary not Italian infinitive).
            # Strip 'per X' purpose clause from GT before storing.
            if (primary_it == kaikki_its[0]
                    and en_norm.startswith("to ")
                    and it_gt and looks_like_it_verb(it_gt)
                    and not looks_like_it_verb(primary_it)):
                primary_it = strip_it_purpose(it_gt)
                it_all = [primary_it]
                stats["verb_noun_overridden"] = stats.get("verb_noun_overridden", 0) + 1
            # Level 2: GT-secondary promotion
            if it_gt and len(kaikki_its) > 1 and primary_it == kaikki_its[0]:
                gt_n = _norm(it_gt)
                secondaries_norm = [_norm(s) for s in kaikki_its[1:]]
                if gt_n in secondaries_norm and gt_n != _norm(primary_it):
                    idx = secondaries_norm.index(gt_n)
                    promoted = kaikki_its[1 + idx]
                    primary_it = promoted
                    it_all = [promoted] + [s for s in kaikki_its if s != promoted][:2]
                    stats["gt_secondary_promoted"] = stats.get("gt_secondary_promoted", 0) + 1
            # Level 5: GT-no-backing override
            if (primary_it == kaikki_its[0]
                    and it_gt and it_freq and _norm(it_gt) != _norm(primary_it)):
                claude_backs = bool(claude_it) and _norm(claude_it) == _norm(primary_it)
                if not claude_backs:
                    it_gt_clean = strip_it_purpose(it_gt)
                    gt_root = _norm(it_gt_clean).rstrip("!-").strip().split()[0]
                    prim_root = _norm(primary_it).rstrip("!-").strip().split()[0]
                    if gt_root and prim_root:
                        prim_rank = it_freq.get(prim_root, 99999)
                        gt_rank = it_freq.get(gt_root, 99999)
                        if gt_rank < prim_rank:
                            primary_it = it_gt_clean
                            it_all = [it_gt_clean]
            ...
            primary_it = claude_it
            it_all = [claude_it]
        else:
            continue

        gt_agrees = it_matches(primary_it, it_gt)
        claude_agrees = it_matches(primary_it, claude_it)
        if gt_agrees:
            stats["gt_agrees"] += 1
        if claude_agrees:
        ...
        entry = {
            "en": en,
            "it": it_all if len(it_all) > 1 else primary_it,
            "rank": en_to_rank.get(en, 9999),
            "confidence": tag,
        }
        if it_gt and it_gt != primary_it:
            entry["it_gt"] = it_gt
        if claude_it and claude_it != primary_it:
            entry["it_claude"] = claude_it
...
def write_review_sample(results, en_to_rank):
    ...
    lines = ["# EN→IT Review Sample (50 entries)\n",
