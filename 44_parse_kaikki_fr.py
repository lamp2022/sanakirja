#!/usr/bin/env python3
"""44_parse_kaikki_fr.py — same logic as 36_parse_kaikki_de.py but for French."""
...
KAIKKI_FILE = "../sanakirja/kaikki_french.jsonl"
OUTPUT_FILE = "../sanakirja/kaikki_fr_en_fr.json"
            ...
            for sense in d.get("senses", []) or []:
                for g in sense.get("glosses", []) or []:
                    for en in extract_primary_en_terms(g):
                        ...
                        if fr_lemma not in plist:
                            plist.append(fr_lemma)
                            n_kept += 1

    print(f"\n  Total FR entries: {n_entries:,}")
    print(f"  EN→FR mappings:   {n_kept:,}")
